...
class ColorStyle:
    def __init__(self, color, link_pairs, point_color):
        ...
        for i in range(len(self.color)):
            self.link_pairs[i].append(tuple(np.array(self.color[i]) / 255.0))

        self.ring_color = []
        for i in range(len(self.point_color)):
            self.ring_color.append(tuple(np.array(self.point_color[i]) / 255.0))
...
def parse_args():
    ...
    parser.add_argument(
        "--prediction", help="Prediction file to visualize", type=str, required=True
    )
                ...
                for i, dt in enumerate(dts):
                    ...
                    if iou < 0.1 or score < threshold:
                        ...
                        for k, link_pair in enumerate(link_pairs):
                            if (
                                link_pair[0] in joints_dict
                                and link_pair[1] in joints_dict
                            ):
                                if (
                                    dt_joints[link_pair[0], 2] < joint_thres
...
if __name__ == "__main__":
    ...
    plot(
        ...
        img_path,
        save_path,
        colorstyle.link_pairs,
        colorstyle.ring_color,
        save=True,
    )
