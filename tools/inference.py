...
def get_pose_estimation_prediction(pose_model, image, centers, scales, transform):
    ...
    coords, _ = get_final_preds(
        cfg, output.cpu().detach().numpy(), np.asarray(centers), np.asarray(scales)
    )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        const=True,
        default=False,
        help="Activate visualization of the inference as a video",
    )
    ...
    parser.add_argument(
        ...
        const=True,
        default=False,
        help="Visualize boxes on the output video",
    )
